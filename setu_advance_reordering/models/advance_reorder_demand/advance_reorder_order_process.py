...
class AdvanceReorderOrderProcess(models.Model):
    ...
    def onchange_vendor_selection_strategy(self):
        """
              use: reset vendor when strategy is not specific vendor.
        """
    ...
    def get_history_sales(self, products, warehouses, start_date, end_date):
        """
              added by: Aastha Vora | On: Oct - 15 - 2024 | Task: 998
              use: used to get history sales data.
        """
    ...
    def action_reorder_confirm(self):
        """
               added by: Aastha Vora | On: Oct - 15 - 2024 | Task: 998
               use: Validate button action to validate reorder and generate demand.
        """
    ...
    def _filter_supplier_info_by_moq(self, ps_info, total_demand):
        """Pick supplier info line applying reorder minimum quantity rules."""
        ...
        if ps_info_have_min_qty:
            ps_info_have_min_qty = ps_info_have_min_qty.filtered(
                lambda x: x.reorder_minimum_quantity <= total_demand
    ...
    def _get_product_supplier_info(self, product, company_id, total_demand):
        """Resolve vendor supplier info using vendor selection strategy."""
        self.ensure_one()
        partner = False
        if self.vendor_selection_strategy == 'specific_vendor':
                ...
                if ps_info:
                    if company_id and ps_info.currency_id != company_id.currency_id:
                        supplier_info_price = ps_info.currency_id._convert(
                            ps_info.price, company_id.currency_id, company_id,
                            self.reorder_date or fields.Date.context_today(self), False)
                    else:
                        supplier_info_price = ps_info.price
                    vendor_moq = ps_info.reorder_minimum_quantity
                else:
                    supplier_info_price = product.standard_price
                    vendor_moq = 0
                reorder_total_amount.append(round(order_qty) * supplier_info_price)
                order_qty_in_purchase_uom = round(
                    product.uom_id._compute_quantity(qty=order_qty, to_unit=product.uom_po_id))

                if ps_info and total_demand < ps_info.reorder_minimum_quantity:
    ...
    def _prepare_purchase_order_line_vals(self, fpos, warehouse_group_id, partner=None, summary_lines=None):
        """
              added by: Aastha Vora | On: Oct - 15 - 2024 | Task: 998
              use: use to prepare purchase order line vals.
        """
            ...
            if reorder_line:
                ...
                if company_id:
                    ps_info = product_id.seller_ids.filtered(lambda
                                                                 x: x.partner_id.id == partner.id and x.company_id == company_id and x.currency_id == company_id.currency_id)
                else:
                    ps_info = product_id.seller_ids.filtered(lambda x: x.partner_id.id == partner.id)
    ...
    def get_vendor_product_mapping_dict(self):
        """
              use: get products grouped by vendor using the same selection logic as planner.
        """
        self.ensure_one()
        product_ids = self.summary_ids.mapped('product_id')
        vendor_product_dict = {}
        if self.vendor_selection_strategy == 'specific_vendor':
            ...
            for product_id in product_ids:
                ...
                if partner_id in vendor_product_dict:
                    vendor_product_dict[partner_id].append(product_id.id)
                else:
                    vendor_product_dict[partner_id] = list(product_id.ids)
        return vendor_product_dict

    def action_open_po_vendor_wizard(self):
        """Open wizard to assign a vendor per summary line when the reorder has no header vendor."""
        self.ensure_one()
    ...
    def action_create_reorder_purchase_order(self):
        ...
        if not vendor_product_dict:
            raise UserError(_('No suitable vendor found for the selected products.'))

        for vendor_id, product_list in vendor_product_dict.items():
            partner = self.env['res.partner'].browse(vendor_id)
            summary_lines = self.summary_ids.filtered(lambda s: s.product_id.id in product_list)
