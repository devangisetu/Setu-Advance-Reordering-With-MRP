...
class AdvanceReorderPlanner(models.Model):
    ...
    def onchange_vendor_selection_strategy(self):
        """
              ...
              use: set vendor_id false if the selection strategy if other than specific vendor.
        """
            ...
            if record.vendor_id:
                products = self.env['product.supplierinfo'].sudo().search(
                    ...
                    'product_variant_ids').filtered(lambda x: x.active != False and x.purchase_ok != False and (
                            x.company_id.id in company_ids or not x.company_id))
    ...
    def get_vendor_product_mapping_dict(self):
        """
               added by: Aastha Vora | On: Oct - 16 - 2024 | Task: 998
               use: get products as per the selected vendor.
        """
        ...
        elif self.vendor_selection_strategy == 'without_vendor':
            if product_ids:
                vendor_product_dict[False] = list(product_ids.ids)
        else:
    ...
    def prepare_vales_for_reorder(self, vendor, product_list):
        ...
        if not vendor:
            ...
            return {
                ...
                'vendor_selection_strategy': reorder_strategy,
    ...
    def create_real_demand_record(self):
        """
             added by: Aastha Vora | On: Oct - 16 - 2024 | Task: 998
             use: Run Manually button action use to create Real Demand Record.
        """
            ...
            if self.state == 'verified':
                if self.auto_workflow_id.create_po:
                    if rec.vendor_id:
                        ...
                        rec.message_post(
                            body=_('Purchase orders were not created automatically because no vendor is set on the reorder. Open the record and use Create PO to assign a vendor per product.')
                        )
