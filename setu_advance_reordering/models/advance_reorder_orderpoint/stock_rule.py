...
class StockRule(models.Model):
    ...
    def _run_buy(self, procurements):
        ...
        for procurement, rule in procurements:
            ...
            if order_point.vendor_selection_strategy == "specific_vendor" and order_point.partner_id:
                ...
                sort_by = "delay"
                lead_days, dummy = rule._get_lead_days(procurement.product_id.with_context(sort_by=sort_by))
            ...
            if not supplier:
                supplier = procurement.product_id.with_context(
                    {'sort_by': sort_by, 'op_company': procurement.company_id})._select_seller(
            ...
            for procurement in procurements:
                ...
                if po_line:
                    ...
                    po_line_values.append(self.env['purchase.order.line']._prepare_purchase_order_line_from_procurement(
                        product_id=procurement.product_id, product_qty=procurement.product_qty,
                        product_uom=procurement.product_uom, location_dest_id=procurement.location_id, name=False,
                        origin=False, company_id=procurement.company_id,values=procurement.values, po=po))
    ...
    def get_product_stock(self, product_id, warehouse_id):
        wh_available = product_id.with_context({'warehouse_id': warehouse_id.id}).qty_available
        wh_outgoing = product_id.with_context({'warehouse_id': warehouse_id.id}).outgoing_qty
    ...
    def create_iwt_lines(self, channel, procurements, place='iwt'):
        ...
        for product, qty in product_dict.items():
            ...
            wh_available_stock = product_obj.with_context({'warehouse_id': fulfiller_warehouse.id}).qty_available
            wh_outgoing_stock = product_obj.with_context({'warehouse_id': fulfiller_warehouse.id}).outgoing_qty
