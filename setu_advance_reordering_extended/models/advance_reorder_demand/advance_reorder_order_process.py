...
class AdvanceReorderOrderProcess(models.Model):
    ...
    def _get_order_action(self, product):
        """Determines summary action from BOM type, routes and classification.
    ...
    def get_vendor_product_mapping_dict(self, purchase_summaries, order_action='purchase'):
        """Groups products by vendor for purchase or subcontracting summaries."""
    ...
    def _create_purchase_orders_from_vendor_mapping(self, summaries, order_action='purchase'):
        """Create POs from vendor→product mapping for the given summary lines."""
        ...
        for vendor_id, product_list in vendor_product_dict.items():
            ...
            vendor_summary_lines = summaries.filtered(
    ...
    def action_create_reorder_purchase_order(self, order_action='purchase'):
        """Create purchase/subcontract orders from matching summary lines."""
        self.ensure_one()
        is_subcontracting = order_action == 'subcontracting'
        if is_subcontracting:
            if self.is_subcontracting_created:
                raise UserError(_(
                    'Subcontracting orders have already been created for this reorder process.'
                ))
        ...
        if is_subcontracting:
            processed_summaries = self._create_purchase_orders_from_vendor_mapping(
                summaries, order_action='subcontracting'
            )
        ...
        processed_summaries = self._create_purchase_orders_from_vendor_mapping(
            summaries, order_action='purchase'
        )
        self._mark_summary_lines_done(processed_summaries)
        self._update_state_after_order_creation()
        return True
    ...
    def action_create_reorder_subcontracting(self):
        """Create subcontract POs using the shared purchase-order creation flow."""
        return self.action_create_reorder_purchase_order(order_action='subcontracting')
    ...
    def action_open_po_vendor_wizard(self):
        """Open vendor wizard for purchase summaries (On PO Creation)."""
        self.ensure_one()
