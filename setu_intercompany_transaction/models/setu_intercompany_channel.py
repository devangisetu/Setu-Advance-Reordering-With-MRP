...
class SetuInterCompanyChannel(models.Model):
    ...
    def get_channel_source_for_ict(self, requestor_company_id, all_channel=False):
        if not all_channel:
            channel = self.sudo().search([('requestor_company_id', '=', requestor_company_id)], order='sequence', limit=1)
        else:
            channel = self.sudo().search([('requestor_company_id', '=', requestor_company_id)], order='sequence')
