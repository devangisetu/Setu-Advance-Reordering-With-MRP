...
class SaleOrder(models.Model):
    ...
    sale_iwt_option = fields.Selection([('as_per_company', 'As Per Company'),
                                        ('stock_plus_interwarehouse',
                                         'First use available stock and create inter warehouse transfer for remaining'),
                                        ...
                                        ], string="Create Inter Warhouse Transfer", help="""
    ...
    ict_transfer_type = fields.Selection(string="ICT Transfer Type", related="ict_ids.transfer_type")
    ...
    def enouogh_stock_available(self, single_or_all='check_single'):
        ...
        for line in self.order_line:
            stock = line.product_id.with_context({'warehouse_id': self.warehouse_id.id}).free_qty
    ...
    def create_multi_intercompany_transfer_records(self):
        ...
        for line in self.order_line:
            ...
            product = line.product_id if line.product_id.type != 'service' else self.env['product.product']
        ...
        if remaining_product_dict:
            filtered_keys = [k for k in remaining_product_dict.keys() if k is not False]
            products = self.env['product.product'].browse(filtered_keys)
            self.message_post(
                body=Markup('<p style="color:red;">%s %s %s</p>') % (
                    _('Stock of'),
                    _(str(products.mapped('name'))),
                    _('has been not found in any Companies!')))

    def get_product_stock(self, product_id, order_qty):
        wh_available = product_id.with_context({'warehouse_id': self.warehouse_id.id}).qty_available
        wh_outgoing = product_id.with_context({'warehouse_id': self.warehouse_id.id}).outgoing_qty
    ...
    def create_multi_interwarehouse_transfer_records(self):
        ...
        if remaining_product_dict:
            products = self.env['product.product'].browse(remaining_product_dict.keys()).filtered(
                lambda p: p.is_storable == True)
            self.message_post(body=Markup('<p style="color:red;">%s %s %s</p>') % (_('Stock of'),
                                                                                   _(str(products.mapped('name'))),
